 ...
 */
export function emptyFieldAlert(toastStore: { trigger: (arg0: { message: string; background: string; timeout: number; }) => void; }): void {
    console.log("Clicked");
    toastStore.trigger({
 ...
 * @email zlac318@live.rhul.ac.uk
 */
export function updateType(toastStore: { trigger: (arg0: { message: string; background: string; timeout: 
    number; }) => void; }, NewTypeName: string, id: string, modalStore: { close: () => void; }): void {
    console.log("Clicked");
    if (NewTypeName == '') {
 ...
 * @email zlac318@live.rhul.ac.uk
 */
export function addMetadataField(toastStore: { trigger: (arg0: { message: string; background: 
    string; timeout: number; }) => void;}, NewTypeFields: any[], fieldListable: boolean): any {
    console.log("Clicked");
    ...
    if (name == '') {
        ...
        return NewTypeFields = [...NewTypeFields, { field: name, dataType: type, list: fieldListable }];
    }
}
...
export function removeField(
    event: MouseEvent & { currentTarget: EventTarget & HTMLButtonElement }, NewTypeFields: any[]
): any {
    console.log("Clicked");
    ...
    if (inputs && select) {
        ...
        return NewTypeFields = NewTypeFields.filter((field: { field: string; }) => field.field != name);
    }
}
