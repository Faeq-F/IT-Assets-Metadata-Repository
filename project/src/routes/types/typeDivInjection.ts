	...
	return fetchDocuments('AssetType').then((assetTypeDocuments) => {
		...
		if (assetTypeDocuments.length > 0) {
			for (let i of assetTypeDocuments) {
				let temp = '<div data-assettypeid="'+i._id+'" style="z-index:1;position: relative;padding:10px;border-radius:5px;background-color: #ffffff; box-shadow: rgba(50, 50, 93, 0.25) 0px 2px 5px -1px,rgba(0, 0, 0, 0.3) 0px 1px 3px -1px; margin: 10px;" style="padding: 20px; margin: 10px;"><div class="h3" style="margin:10px; font-weight: bold;">' +
					i.typeName +
				...
				for (let j of i.metadataFields) {
					temp += "<li style='margin-top:5px;margin-left:20px;'>⦿ " + j.field+`<span class='card assetCard variant-ghost-surface' style='margin-left: 5px;padding: 2px 5px;'>${j.dataType}</span>`;

					if (j.list) {
						temp+="<span class='card assetCard variant-ghost-surface' style='margin-left: 5px;padding: 2px 5px;'>Multi-value</span>"
					}
...
function addDeleteHandlers() {
	...
	for (let i of types) {
		...
		if (buttons.length > 0) {
			...
			deleteButton.addEventListener("click", function () { deleteAssetType(typeID); })
		}
