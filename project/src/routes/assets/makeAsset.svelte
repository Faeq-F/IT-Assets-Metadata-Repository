<script lang="ts">
	...
	async function makeAsset() {
		...
		insertDocument('Asset', data).then((response: any) => {
			console.log(response);
		});

		// fetching autogenerated id of created asset from new Asset
		var id: any;
		var assets;

		try {
			assets = fetchDocuments('Asset');
			for (let i of await assets) {
				if (i.assetName == assetObject.assetName) {
					id = i._id;
				}
			}
			console.log(id);
